...
(function (jQuery) {
    ...
    function build(options) {
        ...
        function build() {
            ...
            var minDays = 40; //Math.floor((opts.slideWidth / opts.cellWidth)  + 5);
    ...
    var Chart = function(div, opts) {
        ...
        function addHzHeader(div, dates, cellWidth) {
            ...
            for (var y in dates) {
                for (var m in dates[y]) {
                    console.log("length" + dates[y][m].length);
        ...
        function addBlocks(div, data, cellWidth, start) {
            ...
            for (var i = 0; i < data.length; i++) {
                var o_series = data[i];
                var o_size = DateUtils.daysBetween(o_series.start, o_series.end) + 1;
                var o_offset = DateUtils.daysBetween(start, o_series.start);
                var o_block = jQuery("<div>" , {
                    "class": "ganttview-block",
                    "title": o_series.status,
                    "css": {
                            "width": ((o_size * cellWidth) - 9) + "px",
                            "margin-left": ((o_offset * cellWidth) + 3) + "px"
                            
                        }
                });
                var o_blockData = { id: data.id, name: data.name };
                jQuery.extend(o_blockData, o_series);
                o_block.data("block-data", o_blockData);
                jQuery(rows[rowIdx]).append(o_block);
                o_block.append(jQuery("<div>", { "class": "ganttview-block-text" }).text(o_series.status));
                rowIdx = rowIdx + 1;
                for (var j = 0; j < data[i].series.length; j++) {
                    ...
                    if (data[i].series[j].allocation == "100%") {
                        block.css("background-color", "#D8EDA3");
                    }
                    else if (data[i].series[j].allocation == "50%") {
                        block.css("background-color", "#FCD29A");
                    }
                    else {
                        block.css("background-color", "#F9C4E1");
                    }
                    block.append(jQuery("<div>", { "class": "ganttview-block-text" }).text(data[i].series[j].allocation));
    ...
    var DateUtils = {
        ...
        getBoundaryDatesFromData: function (data, minDays) {
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                if (i == 0) { minStart = start; maxEnd = end; }
